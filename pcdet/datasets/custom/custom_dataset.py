...
class CustomDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        # else:
        #     self.split = dataset_cfg.DATA_SPLIT['test'] # ghjin
    ...
    def include_data(self, mode): # ghjin
        self.logger.info('Loading Custom dataset.')
        custom_infos = []
        for info_path in self.dataset_cfg.INFO_PATH[mode]: # ghjin
    ...
    def get_infos(self, class_names, num_workers=4, has_label=True, sample_id_list=None, num_features=4):
        ...
        def process_single_scene(sample_idx):
            # print('%s sample_idx: %s' % (self.split, sample_idx)) # ghjin
            info = {}
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        for k in tqdm(range(len(infos))):
            # print('gt_database sample: %d/%d' % (k + 1, len(infos))) # ghjin
            info = infos[k]
            sample_idx = info['point_cloud']['lidar_idx']
            points = self.get_lidar(sample_idx)
            annos = info['annos']
...
def create_custom_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    train_split, val_split, test_split = 'train', 'val', 'test' #ghjin
    ...
    test_filename = save_path / ('custom_infos_%s.pkl' % test_split) # ghjin
    ...
    print('Custom info train file is saved to %s' % test_filename)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_custom_infos':
        ...
        case = dataset_cfg['DATA_PATH'].split('/')[-1]
        # ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        ROOT_DIR = (Path(__file__).resolve().parent / '../../../../').resolve()
        create_custom_infos(
            dataset_cfg=dataset_cfg,
            # class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            class_names = ['vehicle', 'bus', 'truck', 'policecar', 'ambulance', 'schoolbus', 'othercar', 'motorcycle', 'bicycle', 'twowheeler', 'pedestrian', 'trafficsign', 'trafficlight'],

            # data_path=ROOT_DIR / 'data' / 'openpcdet' / case,
            # save_path=ROOT_DIR / 'data' / 'openpcdet' / case,
            data_path=ROOT_DIR / 'data' / case,
            save_path=ROOT_DIR / 'data' / case,
        )
